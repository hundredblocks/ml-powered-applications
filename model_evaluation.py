...
def get_calibration_plot(predicted_proba_y, true_y):
    ...
    clf_score = brier_score_loss(true_y, predicted_proba_y,
                                 pos_label=true_y.max())
    print("\tBrier: %1.3f" % clf_score)

    fraction_of_positives, mean_predicted_value = \
        calibration_curve(true_y, predicted_proba_y, n_bins=10)

    ax1.plot(mean_predicted_value, fraction_of_positives, "s-",
             label="Brier score: (%1.3f)" % clf_score)

    ax2.hist(predicted_proba_y, range=(0, 1), bins=10,
             histtype="step", lw=2)
